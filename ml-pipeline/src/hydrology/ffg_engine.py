...
def dynamic_rainfall_threshold(
    ...
    twi_low: float | None = None,
    twi_high: float | None = None,
) -> pd.Series:
    """
    ...
    twi_low/twi_high: optionally pass fixed TWI normalization bounds (e.g.
    the 5th/95th percentile over the WHOLE grid) instead of recomputing them
    from `twi` itself. Required whenever `twi` only spans a handful of cells
    (e.g. scoring one ward's cells at a time) -- with too little spread,
    quantile(0.05) == quantile(0.95) and normalization divides by zero.
    ffg_engine.py's own per-timestamp, whole-grid callers are unaffected:
    they naturally have thousands of cells and keep computing bounds
    per-call unless told otherwise.
    """
    slope_norm = _normalize(slope_deg, low=0, high=45)
    twi_norm = _normalize(
        twi,
        low=twi.quantile(0.05) if twi_low is None else twi_low,
        high=twi.quantile(0.95) if twi_high is None else twi_high,
    )
