...
def build_training_table(
    ...
    sql = """
        SELECT
            ...
            -- EXISTS, not a JOIN on fec.cell_id, so a cell that was flagged in
            -- MULTIPLE historical events (common once a site has more than one
            -- flood on record) doesn't fan out its rainfall row once per event
            -- it was ever part of -- that previously duplicated every such
            -- cell's row count-of-its-events-times over, corrupting the
            -- train/test split with near-duplicate leakage. was_flooded is
            -- still only 1 for timestamps actually inside a matching event's
            -- window, never for every timestamp ever recorded for that cell.
            CASE WHEN EXISTS (
                SELECT 1 FROM flood_event_cells fec
                JOIN flood_events fe ON fe.event_id = fec.event_id
                WHERE fec.cell_id = r.cell_id
                    AND r.observed_at BETWEEN fe.event_start AND COALESCE(fe.event_end, fe.event_start + INTERVAL '1 day')
            ) THEN 1 ELSE 0 END AS was_flooded
        FROM rainfall r
