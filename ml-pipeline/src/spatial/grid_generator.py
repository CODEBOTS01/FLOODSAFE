...
def generate_grid(
    ...
    # Polygon-box intersected with a MultiPolygon boundary comes back typed
    # MultiPolygon even for a single-part result (a GEOS/shapely quirk), but
    # grid_cells.geom is a plain Polygon column — explode so every row is a
    # true Polygon. Assign cell_code after exploding so a genuinely
    # multi-part cell (rare, e.g. a sliver split by a concave boundary) still
    # gets unique codes rather than colliding.
    grid = grid.explode(index_parts=False)
