...
def compute_terrain_rasters(dem_path: str, output_dir: str, stream_threshold: float = 1000.0) -> dict[str, str]:
    ...
    """
    # wbt.work_dir must be absolute: WhiteboxTools resolves any relative
    # output/input filename against it, so an already-relative work_dir plus
    # an output path that ALSO includes that same relative prefix (the
    # previous version of this function) silently doubles the directory
    # (out_dir/out_dir/slope.tif) and every raster fails to write where this
    # function later expects to find it. Filenames passed to wbt.* calls must
    # therefore be bare (no directory component) — work_dir supplies that.
    ...
    filenames = {
        ...
        "twi": "twi.tif",
        "streams": "streams.tif",
        "distance_to_stream": "distance_to_stream.tif",
    }
    paths = {key: str(out_dir / name) for key, name in filenames.items()}

    wbt.slope(dem, filenames["slope"])
    wbt.aspect(dem, filenames["aspect"])
    wbt.d8_pointer(dem, filenames["flow_direction"])
    wbt.d8_flow_accumulation(dem, filenames["flow_accumulation"], out_type="cells")

    wbt.wetness_index(
        sca=filenames["flow_accumulation"],
        slope=filenames["slope"],
        output=filenames["twi"],
    )

    # zero_background=True is required: ExtractStreams defaults to leaving
    # every non-stream cell as NoData, and EuclideanDistance treats NoData
    # input cells as "nothing to compute" rather than "background, find the
    # nearest target" -- so distance_to_stream would only ever get a value
    # (trivially 0) at the stream cells themselves, NoData everywhere else.
    wbt.extract_streams(
        filenames["flow_accumulation"], filenames["streams"], threshold=stream_threshold, zero_background=True
    )
    wbt.euclidean_distance(filenames["streams"], filenames["distance_to_stream"])
