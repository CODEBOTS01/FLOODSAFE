...
def upsert_dataframe(
    ...
    # .where() on a float64 column coerces None back to NaN to preserve the
    # numeric dtype, so NaN never actually becomes SQL NULL and later fails
    # on any NOT-float column (e.g. an INTEGER column) with "integer out of
    # range" when psycopg2 tries to bind a real NaN. Cast to object first so
    # None can actually be stored per-cell.
    records = df.astype(object).where(pd.notnull(df), None).to_dict(orient="records")

    # A single INSERT ... VALUES binds every cell as its own parameter, and
    # PostgreSQL caps a statement at 65535 parameters total. At grid-cell
    # scale (400k+ rows) one INSERT would need millions, so batch it —
    # chunk size keeps params per batch comfortably under that limit
    # regardless of column count.
    chunk_size = max(1, 60000 // max(1, len(df.columns)))
...
def bulk_update(
    ...
    with engine.begin() as conn:
        # See the matching comment in upsert_dataframe: .astype(object) is
        # required or NaN never actually becomes SQL NULL on float columns.
        df[cols].astype(object).where(pd.notnull(df[cols]), None).to_sql(
